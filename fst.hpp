...
inline bool FST::amacLookup(const char keyByte, level_t level,
  ...
  if (level < getSparseStartLevel()) {  // lookup in LoudsDense
    bool keyExistentSoFar =
        louds_dense_->findNextNodeOrValue(keyByte, node_number);
    // if (!keyExistentSoFar) { std::cout << "key does not exist in dense
    // levels" << std::endl; }
    return keyExistentSoFar;
  } else {  // lookup in LoudsSparse
    bool keyExistentSoFar =
        louds_sparse_->findNextNodeOrValue(keyByte, node_number);
    // if (!keyExistentSoFar) { std::cout << "key does not exist in sparse
    // levels" << std::endl; }
    return keyExistentSoFar;
  }
}
...
std::string FST::getMemoryUsageDetails() const {
  return louds_dense_->getMemoryUsageDetails();
}
